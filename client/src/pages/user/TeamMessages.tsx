...
import { MessageSquare, Send, Users, Paperclip } from "lucide-react";
...
export default function TeamMessages() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        <h2 className="text-2xl sm:text-3xl font-bold flex flex-wrap items-center gap-2" data-testid="heading-team-messages">
      ...
      {user?.role === 'team_leader' && (
        <>
          {teamInfo && (
            <Card className="bg-muted/50" data-testid="card-team-info">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 text-sm">
                  <Users className="h-4 w-4 text-muted-foreground" />
                  <span className="text-muted-foreground">Your team:</span>
                  <span className="font-medium" data-testid="text-team-member-count">
                    {teamInfo.members.length} member{teamInfo.members.length !== 1 ? 's' : ''}
                  </span>
                </div>
                {teamInfo.members.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2" data-testid="list-team-members">
                    {teamInfo.members.map((member) => (
                      <span key={member.id} className="text-xs bg-background px-2 py-1 rounded-md" data-testid={`badge-member-${member.id}`}>
                        {member.displayName}
                      </span>
                    ))}
                  </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground" data-testid="text-send-helper">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base sm:text-lg" data-testid="text-messages-count">
        ...
        <CardContent>
          <div className="space-y-2">
            {teamMessages.length > 0 ? (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="text-xs sm:text-sm font-medium" data-testid={`text-sender-${msg.id}`}>
                          {msg.sender?.displayName || 'Team Leader'}
                        </span>
                        {msg.attachments && msg.attachments.length > 0 && (
                          <span className="text-xs text-muted-foreground flex items-center gap-1" data-testid={`text-attachments-${msg.id}`}>
                            <Paperclip className="h-3 w-3" />
                            {msg.attachments.length} attachment{msg.attachments.length !== 1 ? 's' : ''}
                          </span>
                        )}
                      </div>
                      <p className="text-xs sm:text-sm text-foreground whitespace-pre-wrap break-words" data-testid={`text-message-content-${msg.id}`}>
                        {msg.message}
                      </p>
                    </div>
                    <span className="text-xs text-muted-foreground font-mono whitespace-nowrap" data-testid={`text-timestamp-${msg.id}`}>
              ...
              <div className="text-center py-8 text-muted-foreground text-sm" data-testid="text-empty-state">
                {user?.role === 'team_leader' 
