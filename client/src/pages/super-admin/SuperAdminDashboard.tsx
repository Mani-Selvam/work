...
export default function SuperAdminDashboard() {
  ...
  const filteredCompanies = companiesWithStats.filter((item) => {
    const matchesSearch = item.company.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.company.email.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.company.serverId.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="super-admin-dashboard">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold" data-testid="page-title">Super Admin Dashboard</h1>
          <p className="text-muted-foreground">Manage companies, users, and view analytics</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card data-testid="card-total-companies">
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-total-companies">{analytics?.totalCompanies || 0}</div>
            <p className="text-xs text-muted-foreground">
              {analytics?.activeCompanies || 0} active, {analytics?.suspendedCompanies || 0} suspended
            </p>
          </CardContent>
        </Card>
        ...
        <Card data-testid="card-total-users">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {analytics?.totalAdmins || 0} admins, {analytics?.totalMembers || 0} members
            </p>
          </CardContent>
        </Card>
        ...
        <Card data-testid="card-total-payments">
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-total-payments">{analytics?.totalPayments || 0}</div>
        ...
        <Card data-testid="card-total-revenue">
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-total-revenue">₹{analytics?.totalRevenue || 0}</div>
            <p className="text-xs text-muted-foreground">From slot purchases</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <Tabs value={filterStatus} onValueChange={(v) => setFilterStatus(v as typeof filterStatus)} data-testid="tabs-company-filter">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredCompanies.map(({ company, userCount, adminCount, memberCount }) => (
              <Card key={company.id} data-testid={`card-company-${company.id}`} className="hover:shadow-lg transition-shadow">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" data-testid={`button-menu-${company.id}`}>
                ...
                <CardContent>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium" data-testid={`text-user-count-${company.id}`}>{userCount}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Admins:</span>
                      <span className="font-medium" data-testid={`text-admin-count-${company.id}`}>{adminCount} / {company.maxAdmins}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Members:</span>
                      <span className="font-medium" data-testid={`text-member-count-${company.id}`}>{memberCount} / {company.maxMembers}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          {filteredCompanies.length === 0 && (
            <div className="text-center py-12">
              <p className="text-muted-foreground" data-testid="text-no-companies">No companies found matching your search</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent data-testid="dialog-delete-company">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this company? This action cannot be undone and will permanently delete all company data, users, tasks, and reports.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() => selectedCompany && deleteMutation.mutate(selectedCompany)}
